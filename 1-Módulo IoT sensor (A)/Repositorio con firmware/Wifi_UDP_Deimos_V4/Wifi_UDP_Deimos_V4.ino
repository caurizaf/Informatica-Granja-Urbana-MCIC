/*******************************************************************
 *      se realiza ajuste de entregar dentro del mensaje la ip de origen
 *      
 * 
 *******************************************************************/
...
const char* ssid = "URIZA_RUIZ";
const char* password = "Ecopetrol_01";

unsigned int udp_Port = 2390;      // local port to listen on
//IPAddress ip_remote(3, 16, 212, 5);
IPAddress ip_remote(192, 168, 0, 56);
      ...
      if (Serial.available() > 0) 
        {
          msg_wifi = "";
          msg_wifi += "WIFI=S0;";
          msg_wifi += WiFi.localIP().toString();
          msg_wifi += ";";
